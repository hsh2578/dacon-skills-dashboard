...
§4.2의 강한 턴어라운드(+2.5)는 그 이익이 **반복 가능(recurring)**할 때만 인정합니다. 당해 이익 급증이 단발 이벤트(신작 출시·일회성 수주·자산 매각)에서 나왔고, risks 또는 undervaluation_cause에 **"선반영", "콘텐츠/차기작 공백", "출시 효과 소멸", "일회성"** 류가 명시되면 그 이익은 **피크 어닝스**이며 Forward PER 압축은 저평가가 아니라 분모(EPS) 착시입니다.

| 조건 | 보정 |
|---|---|
| §4.2에서 +2.5를 받았으나 이익이 위 일회성 신호이고 risks에 '선반영'까지 명시 | +2.5 → +0.5로 하향 후 추가 −1.5 |
| 턴어라운드가 수주잔고·반복 매출·구조적 수요로 뒷받침(지속 가능) | 보정 없음 (정상 +2.5 유지) |

"저평가 해소"는 *정상화 이익 대비 싸고 촉매로 풀린다*를 의미합니다. 출시 스파이크로 Forward PER만 낮아졌고 이미 주가에 반영된(선반영) 종목은 '해소될 저평가'가 아니므로 상위에 둘 수 없습니다. 새 수치 계산 없이 기존 risks/undervaluation_cause의 일회성·선반영 표현 존재 여부로만 판정합니다(환각 방지 일관).
...
**재무 추세 교차검증 (`data/financials/{code}.json` 있을 때)**: 영업이익률(op_margin) 5~6년 추세로 `nature` 판정을 객관 검증합니다. 새 수치 계산 없이 추세 방향만 봅니다.

| 재무 추세 | 보정 |
|---|---|
| 마진 수년째 추세적 하락 + 매출 정체/감소 (구조적 사양화 데이터 확증) | STRUCTURAL이면 유지, TEMPORARY로 적혀 있어도 추가 −1 |
| 마진 안정·회복 (일시적 부진 후 정상화 데이터 확증) | TEMPORARY면 유지 (재무가 회복 지지) |
| 연간 적자/부진인데 최근 분기만 급등(latest_quarter OPM ≫ 연간) | 일회성 피크 → §4.2.1 연동, 가산 금지 |

재무 파일이 없으면 본 교차검증은 생략하고 `nature`만 사용합니다.
